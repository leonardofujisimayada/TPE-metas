...
#===============================================================
#= Filtro de estudantes do 3ª série do Ensino Fundamental - 2013 = 
#===============================================================
...
# Tabela final
tb_2013_12ano <- merge(x=tb_subgrupos, y=saeb_13_dta_aj_racacor_reshaped, by.x='CO_MUNICIPIO', by.y='id_municipio', all.x=T)
tb_2013_12ano <- merge(x=tb_2013_12ano, y=saeb_13_dta_aj_renda_reshaped, by.x='CO_MUNICIPIO', by.y='id_municipio', all.x=T)

tb_2013_12ano <- tb_2013_12ano %>% 
  mutate(
...
#===============================================================
#= Filtro de estudantes do 3ª série do Ensino Fundamental - 2015 = 
#===============================================================
...
# Tabela final
tb_2015_12ano <- merge(x=tb_subgrupos, y=saeb_15_dta_aj_racacor_reshaped, by.x='CO_MUNICIPIO', by.y='id_municipio', all.x=T)
tb_2015_12ano <- merge(x=tb_2015_12ano, y=saeb_15_dta_aj_renda_reshaped, by.x='CO_MUNICIPIO', by.y='id_municipio', all.x=T)

tb_2015_12ano <- tb_2015_12ano %>% 
  mutate(
    DIFF_RACA = round(`APR_ADEQ_Branca/Amarela`-`APR_ADEQ_Preta/Parda/Indígena`, 4),
    DIFF_RENDA = round(`APR_ADEQ_Quintil superior`-`APR_ADEQ_Quintil inferior`, 4),
    ano = 2015,
...
#===============================================================
#= Filtro de estudantes do 3ª série do Ensino Fundamental - 2017 = 
#===============================================================
...
# Tabela final
tb_2017_12ano <- merge(x=tb_subgrupos, y=saeb_17_dta_aj_racacor_reshaped, by.x='CO_MUNICIPIO', by.y='id_municipio', all.x=T)
tb_2017_12ano <- merge(x=tb_2017_12ano, y=saeb_17_dta_aj_renda_reshaped, by.x='CO_MUNICIPIO', by.y='id_municipio', all.x=T)

tb_2017_12ano <- tb_2017_12ano %>% 
  mutate(
    DIFF_RACA = round(`APR_ADEQ_Branca/Amarela`-`APR_ADEQ_Preta/Parda/Indígena`, 4),
    DIFF_RENDA = round(`APR_ADEQ_Quintil superior`-`APR_ADEQ_Quintil inferior`, 4),
    ano = 2017,
...
#===============================================================
#= Filtro de estudantes do 3ª série do Ensino Fundamental - 2019 = 
#===============================================================
...
# Tabela final
tb_2019_12ano <- merge(x=tb_subgrupos, y=saeb_19_dta_aj_racacor_reshaped, by.x='CO_MUNICIPIO', by.y='id_municipio', all.x=T)
tb_2019_12ano <- merge(x=tb_2019_12ano, y=saeb_19_dta_aj_renda_reshaped, by.x='CO_MUNICIPIO', by.y='id_municipio', all.x=T)

tb_2019_12ano <- tb_2019_12ano %>% 
  mutate(
    DIFF_RACA = round(`APR_ADEQ_Branca/Amarela`-`APR_ADEQ_Preta/Parda/Indígena`, 4),
    DIFF_RENDA = round(`APR_ADEQ_Quintil superior`-`APR_ADEQ_Quintil inferior`, 4),
    ano = 2019,
...
#========================================================================
#= Preparação dos dados do 3ª série do Ensino Fundamental para a projeção = 
#========================================================================

# Junção dos dados do 3ª série
tb_12ano <- rbind(tb_2013_12ano, tb_2015_12ano, tb_2017_12ano, tb_2019_12ano)
qt_cluster_mun <- tb_12ano %>% filter(ano==2019) %>% group_by(ano, cluster) %>% reframe(qt_aluno=sum(`QT_ALUNO_Preta/Parda/Indígena`, na.rm=T)+sum(`APR_ADEQ_Branca/Amarela`, na.rm=T)+sum(APR_ADEQ_Outros, na.rm=T))

#============================
#= Projeção raça/cor 3ª série =
#============================

tb_12ano_sna <- tb_12ano %>% 
...
tb_12ano_sna_racacor <- tb_12ano_sna %>% 
  filter(diff_raca > 0) %>% 
...
tb_12ano_sna_racacor <- tb_12ano_sna_racacor %>% left_join(qt_cluster_mun, by="cluster") 

# Fator de redistribuição para o Brasil
total_alunos <- tb_12ano %>% filter(ano==2019) %>% group_by(cluster) %>% reframe(qt_aluno_total = sum(`QT_ALUNO_Branca/Amarela`, na.rm=T)+sum(`QT_ALUNO_Preta/Parda/Indígena`, na.rm=T)+sum(QT_ALUNO_Outros, na.rm=T))
tb_12ano_sna_racacor <- tb_12ano_sna_racacor %>% left_join(total_alunos, by="cluster") 

fator_redistribuicao_brasil <- tb_12ano_sna_racacor %>%
  summarise(
    diff_brasil =
...
ponto_partida_23 <- saeb_12_ef_23 %>% 
...
df_brasil_proj <- tibble(
  ano = c(2023, 2025, 2027, 2029, 2031, 2033, 2035),
  tx_brasil = c(0.05, 0.05, 0.07, 0.1, 0.15, 0.24, 0.38)
)
...
# Cálculo dos fatores de redistribuição por cluster
tb_12ano_sna_renda <- tb_12ano_sna %>% 
  filter(diff_renda > 0) %>% 
  ...
  reframe(tx_2017=median(tx_2017, na.rm = TRUE), tx_2019=median(tx_2019, na.rm = TRUE), qt_municípios=n()) %>% 
  mutate(diff=abs(tx_2019-tx_2017))
tb_12ano_sna_renda <- tb_12ano_sna_renda %>% left_join(qt_cluster_mun, by="cluster") 

# Fator de redistribuição para o Brasil
total_alunos <- tb_12ano %>% filter(ano==2019) %>% group_by(cluster) %>% reframe(qt_aluno_total = sum(`QT_ALUNO_Quintil inferior`, na.rm=T)+sum(`QT_ALUNO_Quintil superior`, na.rm=T)+sum(QT_ALUNO_NA, na.rm=T))
tb_12ano_sna_renda <- tb_12ano_sna_renda %>% left_join(total_alunos, by="cluster") 

fator_redistribuicao_brasil <- tb_12ano_sna_renda %>%
  summarise(
    diff_brasil =
      sum(diff * qt_aluno_total, na.rm = TRUE) /
      sum(qt_aluno_total, na.rm = TRUE)
...
saeb_19_dta <- merge(x=saeb_19_dta, y=q_mun_19_12, by="id_municipio")
qs <- quantile(
  saeb_19_dta[
    in_situacao_censo == 1 & 
      id_serie == 12 & 
...
quintis_inse <- with(
  saeb_12_ef_23 %>% filter(IN_INSE == 1),
  Hmisc::wtd.quantile(
    x = INSE_ALUNO,
...
ponto_partida_23 <- saeb_12_ef_23 %>%
  ...
  mutate(
    ...
    APR_ADEQ = ifelse(
      (PROFICIENCIA_MT_SAEB >= 350) & (PROFICIENCIA_LP_SAEB >= 300),
      1,
      0
    )
  )
...
saeb_13_racacor_12 <- saeb_13_dta %>% 
  filter(in_situacao_censo == 1 & id_serie == 12 & id_dependencia_adm %in% c(1, 2, 3)) %>% 
  mutate(APR_ADEQ = ifelse((proficiencia_mt_saeb >= 350) & (proficiencia_lp_saeb >= 300), 1 * peso_aluno_mt, 0),cor_raca = case_when(cor_raca %in% c("Branca", "Amarela") ~ "Brancos", cor_raca %in% c("Preta", "Parda", "Indígena") ~ "PPI", .default = "Outros")) %>% 
  group_by(cor_raca) %>% 
...
saeb_15_racacor_12 <- saeb_15_dta %>% 
  filter(in_situacao_censo == 1 & id_serie == 12 & id_dependencia_adm %in% c(1, 2, 3)) %>% 
  mutate(APR_ADEQ = ifelse((proficiencia_mt_saeb >= 350) & (proficiencia_lp_saeb >= 300), 1 * peso_aluno_mt, 0), cor_raca = case_when(cor_raca %in% c("Branca", "Amarela") ~ "Brancos", cor_raca %in% c("Preta", "Parda", "Indígena") ~ "PPI", .default = "Outros")) %>% 
  group_by(cor_raca) %>% 
  reframe(p = sum(APR_ADEQ, na.rm=T) / sum(peso_aluno_mt, na.rm=T)) %>% 
...
saeb_17_racacor_12 <- saeb_17_dta %>% 
  filter(in_situacao_censo == 1 & id_serie == 12 & id_dependencia_adm %in% c(1, 2, 3)) %>% 
  mutate(APR_ADEQ = ifelse((proficiencia_mt_saeb >= 350) & (proficiencia_lp_saeb >= 300), 1 * peso_aluno_mt, 0), cor_raca = case_when(cor_raca %in% c("Branca", "Amarela") ~ "Brancos", cor_raca %in% c("Preta", "Parda", "Indígena") ~ "PPI", .default = "Outros")) %>% 
  group_by(cor_raca) %>% 
  reframe(p=sum(APR_ADEQ, na.rm=T) / sum(peso_aluno_mt, na.rm=T)) %>% 
...
saeb_19_racacor_12 <- saeb_19_dta %>% 
  filter(in_situacao_censo == 1 & id_serie == 12 & id_dependencia_adm %in% c(1, 2, 3)) %>% 
  mutate(APR_ADEQ = ifelse((proficiencia_mt_saeb >= 350) & (proficiencia_lp_saeb >= 300), 1 * peso_aluno_mt, 0), cor_raca = case_when(cor_raca %in% c("Branca", "Amarela") ~ "Brancos", cor_raca %in% c("Preta", "Parda", "Indígena") ~ "PPI", .default = "Outros")) %>% 
  group_by(cor_raca) %>% 
  reframe(p = sum(APR_ADEQ, na.rm=T) / sum(peso_aluno_mt, na.rm=T)) %>% 
...
saeb_21_racacor_12 <- saeb_12_ef_21 %>% 
  filter(IN_PUBLICA==1 & IN_SITUACAO_CENSO==1) %>% 
  mutate(cor_raca = case_when(TX_RESP_Q04 %in% c("A", "D") ~ "Brancos", TX_RESP_Q04 %in% c("B", "C", "E") ~ "PPI", .default = "Outros"), APR_ADEQ = ifelse((PROFICIENCIA_MT_SAEB >= 350) & (PROFICIENCIA_LP_SAEB >= 300), 1 * PESO_ALUNO_MT, 0)) %>% 
  group_by(cor_raca) %>% 
  reframe(p=sum(APR_ADEQ, na.rm=T) / sum(PESO_ALUNO_MT, na.rm=T)) %>% 
  mutate(ano=2021)
...
saeb_23_racacor_12 <- saeb_12_ef_23 %>% 
  filter(IN_PUBLICA==1 & IN_SITUACAO_CENSO==1) %>% 
  mutate(cor_raca = case_when(TX_RESP_Q04 %in% c("A", "D") ~ "Brancos", TX_RESP_Q04 %in% c("B", "C", "E") ~ "PPI", .default = "Outros"), APR_ADEQ = ifelse((PROFICIENCIA_MT_SAEB >= 350) & (PROFICIENCIA_LP_SAEB >= 300), 1 * PESO_ALUNO_MT, 0)) %>% 
  group_by(cor_raca) %>% 
  reframe(p=sum(APR_ADEQ, na.rm=T) / sum(PESO_ALUNO_MT, na.rm=T)) %>% 
...
saeb_racacor_12 <- rbind(saeb_13_racacor_12, saeb_15_racacor_12, saeb_17_racacor_12, saeb_19_racacor_12, saeb_21_racacor_12, saeb_23_racacor_12)
...
saeb_13_renda_12 <- saeb_13_dta[
  ...
  .(
    ...
    APR_ADEQ = ifelse((proficiencia_mt_saeb >= 350) & (proficiencia_lp_saeb >= 300), 1 * peso_aluno_mt, 0),
    grupo_renda = fifelse(
      inse_aluno < qs_13_12[1], "Quintil inferior",
      fifelse(
        inse_aluno < qs_13_12[2], "Outros",
        "Quintil superior"
      )
    )
  )
...
saeb_15_renda_12 <- saeb_15_dta[
  ...
  .(
    ...
    APR_ADEQ = ifelse((proficiencia_mt_saeb >= 350) & (proficiencia_lp_saeb >= 300), 1 * peso_aluno_mt, 0),
    grupo_renda = fifelse(
      inse_aluno < qs_15_12[1], "Quintil inferior",
      fifelse(
        inse_aluno < qs_15_12[2], "Outros",
        "Quintil superior"
      )
    )
  )
...
saeb_17_renda_12 <- saeb_17_dta[
  ...
  .(
    ...
    APR_ADEQ = ifelse((proficiencia_mt_saeb >= 350) & (proficiencia_lp_saeb >= 300), 1 * peso_aluno_mt, 0),
    grupo_renda = fifelse(
      inse_aluno < qs_17_12[1], "Quintil inferior",
      fifelse(
        inse_aluno < qs_17_12[2], "Outros",
        "Quintil superior"
      )
    )
  )
...
saeb_19_renda_12 <- saeb_19_dta[
  ...
  .(
    ...
    APR_ADEQ = ifelse((proficiencia_mt_saeb >= 350) & (proficiencia_lp_saeb >= 300), 1 * peso_aluno_mt, 0),
    grupo_renda = fifelse(
      inse_aluno < qs_19_12[1], "Quintil inferior",
      fifelse(
        inse_aluno < qs_19_12[2], "Outros",
        "Quintil superior"
      )
    )
  )
...
# 2021
qs_21_12 <- with(
  saeb_12_ef_21 %>% filter(IN_INSE == 1),
  Hmisc::wtd.quantile(
    x = INSE_ALUNO,
    weights = PESO_ALUNO_INSE,
    probs = c(0.2, 0.8), # inferior e superior
...
saeb_21_renda_12 <- saeb_12_ef_21 %>%
  filter(
    IN_PUBLICA == 1,
    IN_SITUACAO_CENSO == 1,
    IN_INSE == 1
...
saeb_21_renda_12 <- saeb_21_renda_12 %>%
...
saeb_23_renda_12 <- saeb_12_ef_23 %>%
  filter(
    IN_PUBLICA == 1,
    IN_SITUACAO_CENSO == 1,
    IN_INSE == 1
...
saeb_23_renda_12 <- saeb_23_renda_12 %>%
  group_by(grupo_renda) %>%
  summarise(
    p = sum(APR_ADEQ * PESO_ALUNO_INSE, na.rm = TRUE) /
      sum(PESO_ALUNO_INSE, na.rm = TRUE),
...
saeb_renda_12 <- rbind(saeb_13_renda_12, saeb_15_renda_12, saeb_17_renda_12, saeb_19_renda_12, saeb_21_renda_12, saeb_23_renda_12)
